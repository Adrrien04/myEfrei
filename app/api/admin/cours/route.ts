...
export async function POST(req: NextRequest) {
    try {
        ...
        if (!nom || !id_prof || !matiere) {
            return NextResponse.json({ error: "Tous les champs (nom, professeur, matière) sont requis" }, { status: 400 });
        }

        const courseId = generateCourseId();
        console.log(`✅ ID du cours généré : ${courseId} (Longueur: ${courseId.length})`);
...
export async function PUT(req: NextRequest) {
    try {
        ...
        console.log("🟢 PUT /api/admin/cours - Données reçues :", { id, nom, id_prof, matiere });
