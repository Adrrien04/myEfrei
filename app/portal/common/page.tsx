            ...
            <div className="grid gap-6">
              {events.map((event) => (
                  ...
                  >
                    ...
                    <p className="text-gray-600">{new Date(event.date).toLocaleDateString('fr-FR')}</p>
                  </div>
              ))}
            </div>
          </div>
        ...
        {selectedEvent && (
            <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
              <div className="bg-white p-6 rounded-lg max-w-md w-full max-h-[80vh] overflow-y-auto relative">
                ...
                <h2 className="text-2xl font-bold mb-4 text-center">{selectedEvent.title}</h2>
                <p className="text-gray-600 text-center mb-2">
                  {new Date(selectedEvent.date).toLocaleDateString('fr-FR')}
                  <br/>
                  {selectedEvent.time}
                </p>
                <p className="text-gray-700 text-center mb-4">{selectedEvent.location}</p>
