  ...
  const handleCoursClick = async (classe: string) => {
    ...
    try {
      const res = await fetch(
        `/api/classes/${encodeURIComponent(classe)}/eleves`,
      );
      if (!res.ok) {
      ...
      console.error(
        "❌ Erreur inattendue lors de la récupération des élèves :",
        err,
      );
    }
  };
                      ...
                      <input
                        ...
                        onChange={(ev) =>
                          setEleves((prev) =>
                            prev.map((el) =>
                              el.id === e.id
                                ? { ...el, note: parseFloat(ev.target.value) }
                                : el,
                            ),
                          )
                        }
                      ...
                      <textarea
                        ...
                        onChange={(ev) =>
                          setEleves((prev) =>
                            prev.map((el) =>
                              el.id === e.id
                                ? { ...el, commentaire: ev.target.value }
                                : el,
                            ),
                          )
                        }
                    ...
                    <td className="p-3">
                      <button
                        ...
                        onClick={async () => {
                          ...
                          const selectedCours = cours.find(
                            (c) => c.classe === selectedClasse,
                          );
                          ...
                          if (!selectedCours) {
                            setFeedbackMessage(
                              "❌ Cours introuvable pour cette classe.",
                            );
                            return;
                          }
